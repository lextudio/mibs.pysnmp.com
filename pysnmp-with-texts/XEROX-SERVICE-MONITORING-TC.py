...
xcmSvcMonTC.setLastUpdated("1202170000Z")
if mibBuilder.loadTexts:
    xcmSvcMonTC.setOrganization("""\
Xerox Corporation - XCMI Working Group
""")
xcmSvcMonTC.setContactInfo("""\
 XCMI Editors Email: xcmieditors@xerox.com -- --
""")
if mibBuilder.loadTexts:
...
svcMonGeneralGroup -- 2**0 2 : svcMonQueueGroup -- 2**1 4 : svcMonQueueExtGroup
-- 2**2 8 : svcMonServiceGroup -- 2**3 16 : svcMonServiceDetailGroup -- 2**4 32
: svcMonServiceMgmtGroup -- 2**5 Usage: Conforming management agents SHALL
accurately report their support for XCMI Service Monitoring MIB groups.
"""
...
be extended in the future) is given below: 1 : confirmOnError -- 2**0 2 :
confirmOnWarning -- 2**1 4 : confirmOnAbort -- 2**2 8 : confirmOnCancel -- 2**3
16 : confirmOnComplete -- 2**4 32 : confirmOnReceive -- 2**5 64 :
confirmOnDeliver -- 2**6 128 : confirmOnStateChange -- 2**7 256 :
confirmOnStateReasonChange -- 2**8 Usage: Conforming management agents SHALL
accurately report their support for job confirmation reports.
"""
...
future) is given below: 1 : attachmentTIFF -- 2**0 (TIFF/TIF) 2 : attachmentPDF
-- 2**1 (PDF) 4 : attachmentPS -- 2**2 (PS) 8 : attachmentTXT -- 2**3 (TXT) 16
: attachmentPCL -- 2**4 (PCL) 32 : attachmentPRN -- 2**5 (PRN) 64 :
attachmentJPG -- 2**6 (JPG/JPEG) 128 : attachmentXPS -- 2**7 (XPS) 256 :
attachmentPDFA -- 2**8 (PDF-A) Usage: The set of accepted attachment types is
specified by adding together the various PDL values.
"""
...
'XcmHrDevMgmtCommandDataTag', or are as follows -- Service Management Data --
Domain-specific operations that are only used in -- commands sent using
xcmSvcMonServiceMgmtData. * DT [Date and Time] -- (see the
XcmHrDevMgmtCommandDataTag in the XCMI Ext -- to Host Resources TC MIB). XCMI
...
-- syntax 'DisplayString (SIZE (0..255))' -- 'service operation' standard type
(standardized below -- for coherence) -- 'service operation' extended type
(freeform, no standards) -- Generic xcmSvcMonServiceMgmtData tags * TP (Type -
no object mapping) -- syntax 'DisplayString (SIZE (0..255))' * TM (Time - no
object mapping) -- qualifier for SO -- Time for management operation request --
syntax integer (time in seconds) * FN (Filename - no object mapping) --
qualifier for SO -- syntax 'DisplayString (SIZE (0..255))' * CreatePublicFolder
-- requires FN qualifier -- SO=CreatePublicFolder:FN=<filename of the folder to
be created>: * DeleteAllPublicFiles * DeleteAllPublicFolders * DeletePublicFile
-- requires FN qualifier -- SO=DeletePublicFile:FN=<Name of the file to be
deleted>: * DeletePublicFolder -- requires FN qualifier --
SO=DeletePublicFolder:FN=<Filename of the folder to be deleted>: * Disable *
DiskOverwrite -- can be used with qualifiers -- i.e.
SO=DiskOverwrite:TP=std:TM=360: used to request -- 'Standard disk overwrite in
1 hour' * S2DPrivateAccessPasswordOn * S2DPrivateAutoDeleteInterval -- requires
time qualifier in seconds -- SO=S2DPrivateAutoDeleteInterval:TM=<360-86313600>:
* S2DPrivateAutoDeleteOn * S2DPrivateScanPasswordOn *
S2DPublicAutoDeleteInterval -- requires time qualifier in seconds --
SO=S2DPublicAutoDeleteInterval:TM=<360-86313600>: * S2DPublicAutoDeleteOn *
S2DPublicState -- Print operation xcmSvcMonServiceMgmtData tags *
PrintCollationOff * PrintCollationOn * PrintMedia2ndSide * PrintMediaCardStock
* PrintMediaColoredPaper * PrintMediaEnvelope * PrintMediaLabel *
PrintMediaLetterhead * PrintMediaPlainPaper * PrintMediaPreprinted *
PrintMediaPrepunched * PrintMediaSpecial * PrintMediaTransparency *
PrintOrientationLandScape * PrintOrientationPortrait * PrintPlexOff *
PrintPlexOn * PrintStaplingOff * PrintStaplingOn The above XCMI standard 'tags'
